...
static mut __data: Vec<String> = Vec::new();
...
// read lines
    ...
    __data.push(String::from(".section .data"));
    readLines(&mut _lines, &mut _lineIndex, &mut _linesLength);
    __data.push(String::from(".section .text"));
    __data.push(String::from(".globl _start"));
    __data.push(String::from("_start:"));
    __data.append(&mut __text);
    __data.push(String::from("exit:"));
    __data.push(String::from("  movl $1, %eax  # todo: exit func"));
    __data.push(String::from("  xorl %ebx, %ebx  # code 0"));
    __data.push(String::from("  int $0x80"));
    // output result
    //for i in __data.iter() {
    //    println!("{}",i);
    //}
    // write result file
    let file = match File::create("builded.as") {
        Ok(f) => f,
        Err(e) => {
